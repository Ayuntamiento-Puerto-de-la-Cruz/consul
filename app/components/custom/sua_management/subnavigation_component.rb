class SUAManagement::SubnavigationComponent < ApplicationComponent
  attr_reader :current
  ...
  private
    ...
    def text_for(section)
      "SUA::#{section.to_s.classify}".constantize.model_name.human(count: 2).titleize
    end

    def path_for(section)
      {
        controller: "sua_management/#{section}",
        action: :index
      }
    end
